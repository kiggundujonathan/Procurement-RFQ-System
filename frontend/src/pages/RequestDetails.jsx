...
const downloadPDF = () => {
  ...
  doc.text(`Requestor: ${request.requestedBy?.fullName || "N/A"}`, 10, y);
  y += 7;
...
const supervisorRecord =
  request.approvalHistory?.find(
    (h) => h.stage === "Supervisor"
  );

const procurementRecord =
  request.approvalHistory?.find(
    (h) => h.stage === "Procurement"
  );
    ...
    sections: [
      {
        children: [
          ...
          new Paragraph({
            children: [
              new TextRun({
                text: `Requestor: ${request.requestedBy?.fullName || "N/A"}`,
                size: 28,
              }),
            ],
          }),
...
...(request.attachments || []).map(
  ...
  const handleDownload = (attachments) => {
    ...
    a.href = attachments.data;
    a.download = attachments.name;
    a.click();
  };
 return (
  <div
